...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] = searchTerms.concatRecovered(text => wikiSuggestResponseStream(text))
    ...
    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.concatRecovered(pages => wikiPageResponseStream(pages)) 
